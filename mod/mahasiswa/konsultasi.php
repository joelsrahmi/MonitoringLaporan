...
if ($_GET['act']=='dtl'){
	...
	$nimk = getValue("nkel","penugasan","pid='$fk[pid]'");
	$tglk = "Pada ".getTglIndo(substr($fk['kWaktu'], 0,10))." | Jam : ".substr($fk['kWaktu'], 11,8);
	$mhsk = getValue("mNama","mahasiswa","mid='$nimk'");
	$ffoto = getValue("mFoto","mahasiswa","nkel='$nimk'");
	if (empty($ffoto)){
		$fotok = "images/photo.jpg";
	}else{
		$fotok = "logo_kel/$ffoto";
	}
	$fberkas = $fk['kFile'];
			...
			<div class="timeline-container">
				...
				while ($dk = mysql_fetch_array($qkomen)){
					if ($dk['kkLevel']==2){
						...
						$konama = getValue("mid","mahasiswa","mid='$dk[kkOleh]'");
						$kfoto = getValue("mFoto","mahasiswa","mid='$dk[kkOleh]'");
						if (empty($kfoto)){
							$kofoto = "images/photo.jpg";
						}else{
							$kofoto = "logo_kel/$kfoto";
						}
					}
...
<?php
	...
	$uid = $_SESSION['uId'];
	$dsk = mysql_fetch_array(mysql_query("SELECT a.*,b.* FROM penugasan a LEFT JOIN mahasiswa b ON a.nkel=b.nkel WHERE mid='$uid' AND statJudul='2'"));
	$peny = getValue("nama_lengkap","dosen","id_dosen='$dsk[id_dosen]'");
	$pid = $dsk['pid'];
	$speny = $dsk['id_dosen'];
	$npage = $_GET['page'];
?>
<div class="page-header">
	<div class="alert alert-block alert-success">
		<p>
			<strong><?php echo $dsk['judul']?></strong><br><br>
			Penyunting : <?php echo $peny;?><br>
		</p>
	</div>
</div>
<div class="row-fluid">
			...
			<div class="widget-body">
				<div class="widget-main no-padding">
					<form method="POST" enctype="multipart/form-data">
						...
						<fieldset>
							<label><strong>Berkas File</strong></label>
							
							<input type="file" id="myFile" name="fupload" required/>
							<br>
							
							<label><strong>Penyunting</strong></label>
							<div class="controls">
								<select class="chosen-select" name="peny" id="peny" readonly required>
									<?php
										echo "<option value='$speny'>$peny</option>";
									?>
								</select>
								<br>
							</div>
					...
					<?php
						...
						if (isset($_POST['kirim'])){
							...
							$q = mysql_query("INSERT INTO konsultasi (pid,dosId,kFile,kKet) VALUES ('$_POST[pid]',
								'$_POST[peny]','$file','$_POST[ket]')");
		...
		<tbody>
		 <?php
		 	$qry = mysql_query("SELECT a.* FROM konsultasi a
		 							  WHERE a.pid='$pid' ORDER BY a.kId DESC");
